...
//   List<UploadBean> selectByUserId(Integer userid);
   @Query(value="select * from sqsx_upload where user_id=?1 and isdel=0",nativeQuery = true)
   //@Modifying
   List<UploadBean> selectByUserId(Integer userid);
   //List<UploadBean> findByUser_id(Integer userid);
   @Query(value="select * from sqsx_upload where user_id=?1 and file_id=?2 and isdel=0",nativeQuery = true)
   @Modifying
